...
def gen_log(prefix, length, er, delta, seed, aps):
    print("./gen_log logs/" + " ".join(map(str, [prefix, length, er, delta, seed, aps])), file=bash)
...
def gen_er(name):
    log_dict = exps[name]["log"]
    for er in exps[name]["range"]:
        log_name = "_".join(map(str, [name, er]))
        gen_log(log_name, log_dict["len"], er, log_dict["delta"], 0, log_dict["aps"])
        for f in range(exps[name]["fmlas"]):
            gen_fmla(name, er, f, exps[name]["size"], exps[name]["max_int"], exps[name]["type"], 1, exps[name]["aps"])
            gen_log_symlink(log_name, name, er, f)
            run_experiment(name, exps[name]["tools"], er, f)
...
def main():
    for exp_name, exp_dict in exps.items():
        if "log" in exp_dict:
            log_dict = exp_dict["log"]
            gen_log(exp_name, log_dict["len"], log_dict["er"], log_dict["delta"], 0, log_dict["aps"])
